...
class zabbix::agent (
  ...
  $zabbix_package_agent                                = $zabbix::zabbix_package_agent,
  Optional[String[1]] $zabbix_package_provider         = $zabbix::zabbix_package_provider,
  Optional[Stdlib::Windowspath] $zabbix_package_source = undef,
  Boolean $manage_choco                                = $zabbix::manage_choco,
  ...
  $monitored_by_proxy                                  = $zabbix::monitored_by_proxy,
  $agent_use_ip                                        = $zabbix::agent_use_ip,
  $zbx_group                                           = $zabbix::agent_zbx_group,
  Variant[String[1],Array[String[1]]] $zbx_groups      = $zabbix::agent_zbx_groups,
  $zbx_group_create                                    = $zabbix::agent_zbx_group_create,
  $zbx_templates                                       = $zabbix::agent_zbx_templates,
  Array[Hash] $zbx_macros                              = [],
  Integer[1,4] $zbx_interface_type                     = 1,
  Variant[Array, Hash] $zbx_interface_details          = [],
  $agent_configfile_path                               = $zabbix::agent_configfile_path,
  $pidfile                                             = $zabbix::agent_pidfile,
  $servicename                                         = $zabbix::agent_servicename,
  Enum['console', 'file', 'system'] $logtype           = $zabbix::agent_logtype,
  Optional[Stdlib::Absolutepath] $logfile              = $zabbix::agent_logfile,
  $logfilesize                                         = $zabbix::agent_logfilesize,
  $debuglevel                                          = $zabbix::agent_debuglevel,
  $sourceip                                            = $zabbix::agent_sourceip,
  Optional[String[1]] $allowkey                        = $zabbix::agent_allowkey,
  Optional[String[1]] $denykey                         = $zabbix::agent_denykey,
  $enableremotecommands                                = $zabbix::agent_enableremotecommands,
  $logremotecommands                                   = $zabbix::agent_logremotecommands,
  $server                                              = $zabbix::agent_server,
  $listenport                                          = $zabbix::agent_listenport,
  $listenip                                            = $zabbix::agent_listenip,
  $startagents                                         = $zabbix::agent_startagents,
  $serveractive                                        = $zabbix::agent_serveractive,
  Stdlib::Ensure::Service $service_ensure              = $zabbix::agent_service_ensure,
  Boolean $service_enable                              = $zabbix::agent_service_enable,
  $hostname                                            = $zabbix::agent_hostname,
  $hostnameitem                                        = $zabbix::agent_hostnameitem,
  $hostmetadata                                        = $zabbix::agent_hostmetadata,
  $hostmetadataitem                                    = $zabbix::agent_hostmetadataitem,
  Optional[Stdlib::Fqdn] $hostinterface                = $zabbix::agent_hostinterface,
  Optional[Stdlib::Fqdn] $hostinterfaceitem            = $zabbix::agent_hostinterfaceitem,
  $refreshactivechecks                                 = $zabbix::agent_refreshactivechecks,
  $buffersend                                          = $zabbix::agent_buffersend,
  $buffersize                                          = $zabbix::agent_buffersize,
  $maxlinespersecond                                   = $zabbix::agent_maxlinespersecond,
  Optional[Array] $zabbix_alias                        = $zabbix::agent_zabbix_alias,
  $timeout                                             = $zabbix::agent_timeout,
  $allowroot                                           = $zabbix::agent_allowroot,
  Optional[String[1]] $zabbix_user                     = $zabbix::agent_zabbix_user,
  $include_dir                                         = $zabbix::agent_include,
  $include_dir_purge                                   = $zabbix::agent_include_purge,
  $unsafeuserparameters                                = $zabbix::agent_unsafeuserparameters,
  $userparameter                                       = $zabbix::agent_userparameter,
  Optional[String[1]] $loadmodulepath                  = $zabbix::agent_loadmodulepath,
  $loadmodule                                          = $zabbix::agent_loadmodule,
  Optional[Enum['unencrypted','psk','cert']] $tlsaccept = $zabbix::agent_tlsaccept,
  $tlscafile                                           = $zabbix::agent_tlscafile,
  $tlscertfile                                         = $zabbix::agent_tlscertfile,
  Optional[String[1]] $tlscertissuer                   = undef,
  Optional[String[1]] $tlscertsubject                  = undef,
  Optional[String[1]] $tlscipherall                    = $zabbix::agent_tlscipherall,
  Optional[String[1]] $tlscipherall13                  = $zabbix::agent_tlscipherall13,
  Optional[String[1]] $tlsciphercert                   = $zabbix::agent_tlsciphercert,
  Optional[String[1]] $tlsciphercert13                 = $zabbix::agent_tlsciphercert13,
  Optional[String[1]] $tlscipherpsk                    = $zabbix::agent_tlscipherpsk,
  Optional[String[1]] $tlscipherpsk13                  = $zabbix::agent_tlscipherpsk13,
  Optional[Enum['unencrypted','psk','cert']] $tlsconnect = $zabbix::agent_tlsconnect,
  $tlscrlfile                                          = $zabbix::agent_tlscrlfile,
  $tlskeyfile                                          = $zabbix::agent_tlskeyfile,
  $tlspskfile                                          = $zabbix::agent_tlspskfile,
  $tlspskidentity                                      = $zabbix::agent_tlspskidentity,
  $tlsservercertissuer                                 = $zabbix::agent_tlsservercertissuer,
  $tlsservercertsubject                                = $zabbix::agent_tlsservercertsubject,
  Optional[String[1]] $agent_config_owner              = $zabbix::agent_config_owner,
  Optional[String[1]] $agent_config_group              = $zabbix::agent_config_group,
  Boolean $manage_selinux                              = $zabbix::manage_selinux,
  Array[String] $selinux_require                       = $zabbix::selinux_require,
  Hash[String, Array] $selinux_rules                   = $zabbix::selinux_rules,
  String $additional_service_params                    = $zabbix::additional_service_params,
  String $service_type                                 = $zabbix::service_type,
  ...
  if $manage_resources {
    ...
    if $zbx_group == $zabbix::agent_zbx_group and $zbx_groups == $zabbix::params::agent_zbx_groups {
      $groups = $zabbix::agent_zbx_groups
    } else {
      if $zbx_group != $zabbix::agent_zbx_group and $zbx_groups != $zabbix::params::agent_zbx_groups {
      ...
      if $zbx_group != $zabbix::agent_zbx_group {
